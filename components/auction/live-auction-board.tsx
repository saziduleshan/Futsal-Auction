        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-right">
              ...
              <p className="mt-1 text-lg font-bold">{currency(room.bid_increment)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-0 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="border-b border-white/10 p-6 md:border-b-0 md:border-r md:border-white/10 md:p-8">
          {currentPlayer ? (
            <>
              <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/5">
                {currentPlayer.card_image_url ? (
                  ...
                  <div className="flex aspect-[4/5] items-center justify-center bg-gradient-to-br from-cyan/20 via-white/5 to-magenta/20">
            ...
            <div className="flex min-h-[460px] items-center justify-center rounded-[2rem] border border-dashed border-white/10 bg-white/5 p-10 text-center">
              <div>
        ...
        <div className="p-6 md:p-8">
          <div className="glass rounded-3xl p-6">
            ...
            <p className="mt-3 text-4xl font-black uppercase tracking-[0.12em] text-lime">
            ...
            {viewerRole === 'team' ? (
              <button
                ...
                className="mt-6 w-full rounded-2xl bg-lime px-5 py-4 font-black uppercase tracking-[0.18em] text-pitch transition hover:bg-white disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/45"
              >
            ...
            {teams.map((team) => (
              <div key={team.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  {room.current_highest_team_id === team.id ? <span className="badge border-lime/30 text-lime">Leading</span> : null}
                </div>
                <p className="mt-3 inline-flex items-center gap-2 text-sm text-white/65"><Wallet className="h-4 w-4 text-cyan" />Purse {currency(team.purse)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-6">
            ...
            <div className="mt-4 space-y-3">
              {recentBids.length ? recentBids.map((bid) => {
                ...
                return (
                  <div key={bid.id} className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm">
