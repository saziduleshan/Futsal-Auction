      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="panel overflow-hidden p-8">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            {teams.map((team) => (
              <div key={team.id} className="rounded-2xl border border-cyan/10 bg-gradient-to-br from-cyan/[0.04] to-transparent p-4 transition hover:border-cyan/30">
                <p className="font-bold uppercase tracking-[0.08em]">{team.name}</p>
                <p className={`mt-2 text-sm ${team.division === 'men' ? 'text-cyan/60' : 'text-magenta/60'}`}>{team.division === 'men' ? 'Male Futsal' : 'Female Futsal'}</p>
                <p className="mt-3 text-xl font-black text-gold">{currency(team.purse)}</p>
              </div>
            ))}
          </div>
          <div className="mt-6 rounded-2xl border border-purple/20 bg-gradient-to-br from-purple/[0.06] to-transparent p-4 text-sm text-white/65">
