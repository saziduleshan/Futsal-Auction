          ...
          <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-4">
                <Link href="/auction" className="rounded-full bg-gradient-to-r from-lime to-cyan px-6 py-3 font-black uppercase tracking-[0.18em] text-pitch transition hover:from-white hover:to-white">
                  Enter Live Auction
                </Link>
                <Link href="/login" className="rounded-full border border-purple/30 px-6 py-3 font-black uppercase tracking-[0.18em] text-white transition hover:border-purple/50 hover:bg-purple/10">
                  Team / Admin Login
                </Link>
              </div>
              ...
              <div className="mt-10 grid gap-4 md:grid-cols-2">
                {[
                  ...
                  const colors = ['border-cyan/20 from-cyan/[0.06]', 'border-lime/20 from-lime/[0.06]', 'border-magenta/20 from-magenta/[0.06]', 'border-orange/20 from-orange/[0.06]'];
                  return (
                    <div key={item.title} className={`rounded-2xl border bg-gradient-to-br to-transparent p-4 ${colors[idx]}`}>
                      <item.icon className={`h-5 w-5 ${idx === 0 ? 'text-cyan' : idx === 1 ? 'text-lime' : idx === 2 ? 'text-magenta' : 'text-orange'}`} />
            ...
            <div className="space-y-5">
              {DIVISIONS.map((division) => {
                ...
                return (
                  <div key={division.value} className={`panel overflow-hidden p-6 ${division.value === 'men' ? 'border-cyan/20' : 'border-magenta/20'}`}>
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className={`rounded-2xl border px-4 py-3 text-right ${room?.status === 'live' ? 'border-lime/30 bg-lime/[0.06]' : 'border-white/10 bg-white/5'}`}>
                    ...
                    <div className="mt-5 grid gap-3">
                      {divisionTeams.map((team, idx) => (
                        <div key={team.id} className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/20 px-4 py-3 transition hover:border-cyan/30">
                          <span className="inline-flex items-center gap-2 font-semibold"><Users className={`h-4 w-4 ${division.value === 'men' ? 'text-cyan' : 'text-magenta'}`} />{team.name}</span>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 md:px-6 md:pb-24">
        <div className="panel overflow-hidden p-8 md:p-10">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            ...
            <Link href="/admin" className="inline-flex items-center gap-2 rounded-full border border-orange/40 px-5 py-3 font-bold uppercase tracking-[0.16em] text-orange transition hover:bg-orange/10">
